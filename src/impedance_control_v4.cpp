...
int main(int argc, char** argv) {
  ...
  const double xy_velocity_damping{50.0}; // N/(m/s), larger = stronger speed regulation
  const double xy_direction_sign{1.0};    // Change to -1.0 if direction feels reversed.

  // Z force-control parameters.
  const double z_stiffness{50.0};         // N/m
  ...
  const double rotational_stiffness{0.8};
  const double rotational_damping{2.0 * std::sqrt(rotational_stiffness)};
  const double angular_follow_speed{0.8}; // rad/s

  // Conservative Cartesian task-force limiter.
  const double max_xy_task_force{5.0};   // N
